        ...
        <div ref="container" class="vpd-container">
          <div class="vpd-content">
            <div class="vpd-header" :style="{'background-color': color}">
              ...
              <slot v-if="state.locales.length > 1" name="locales">
                <locale-change
            ...
            <div class="vpd-body">
                        ...
                        <span
                          ...
                          v-text="
                            convertToLocaleNumber(
                              state.date.xFormat('jMMMM jYYYY')
                            )
                          "
                        />
                      </slot>
              ...
              <div v-else class="vpd-actions">
                <slot name="submit-btn" v-bind="{canSubmit, submit, lang}">
                  <button
                    type="button"
                    :disabled="!canSubmit"
                    :style="{color}"
                ...
                <slot v-if="!inline" name="cancel-btn" v-bind="{lang}">
                  <button
                    type="button"
                    :style="{color}"
...
export default {
  ...
  setup(props, {emit}) {
    ...
    const getHighlights = function (item, value) {
      ...
      return (
        applyDevFn(highlight, item, value.format(selfFormat), value.clone()) ||
        {}
      )
    }
    ...
    const month = computed(() => {
      ...
      return state.core.getWeekArray(state.date.clone()).map((weekItem) => {
        return weekItem.map((day) => {
          ...
          data.selected = state.selectedDates.find((item) =>
            isSameDay(item, day)
          )
          data.disabled =
    ...
    const months = computed(() => {
      if (hasStep('m')) {
        ...
        let months = state.core.getMonthsList(
          state.minDate,
          state.maxDate,
          date
        )
    ...
    const isDisableTime = computed(
      () => hasStep('t') && checkDisable('t', state.time)
    )
    const timeAttributes = computed(() =>
      hasStep('t') ? getHighlights('t', state.time) : {}
    )
    ...
    const weekDays = computed(() => {
      let names = JSON.parse(
        JSON.stringify(state.core.moment().localeData().weekdaysMin())
      )
    ...
    const isPopover = computed(
      () => (props.popover === '' || props.popover) && state.windowWidth > 480
    )
    ...
    watch(
      [() => props.modelValue, () => props.initialValue, () => props.timezone],
      () => updateDates()
    )
    watch(
      () => props.inline,
    ...
    watch(
      () => state.date,
      (val, oldVal) => {
        state.directionClassDate = setDirection(
          state.directionClassDate,
          val,
          oldVal
        )
